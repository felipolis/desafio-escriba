...
export function searchProducts({ commit }, keyword='') {
    axios.get('http://localhost:3000/produtos')
        .then(({ data }) => {
            if (keyword){
                const products = data.filter(product => product.nome.toLowerCase().includes(keyword.toLowerCase()))
...
export function searchOrders({ commit }, keyword='') {
    axios.get('http://localhost:3000/pedidos')
        .then(({ data }) => {
            if (keyword){
                const orders = data.filter(order => order.nome.toLowerCase().includes(keyword.toLowerCase()))
